...
> Prompt de integração para o repo `Maxwbh/Gestao-Contrato`.
...
## Resiliência ao cold start do BRCobrança (Render free) — HTTP 429

**Sintoma real (produção):** ao gerar remessa, o `cnab_service` recebe `429 Too
Many Requests` da BRCobrança e mostra *"API sobrecarregada (limite de requisições
atingido)"*. **Não é rate limit** — a API BRCobrança não tem rate limiting. É o
**cold start do Render free**: após ~15 min ocioso o serviço dorme e o primeiro
acesso leva **~20-30s** para acordar; nessa janela o Render responde `429`.

**O retry atual é curto demais.** Os logs mostram `1s → 2s → desiste (3/3)` em
~6s, enquanto o wake leva ~30s. Corrigir o `cnab_service`:

1. **Warm-up antes do lote:** chamar `GET {BOLETO_API_URL}/api/health` com
   `timeout=90s` e só seguir após `200`. Uma chamada acorda o serviço para todo
   o lote (evita 429 conta a conta).
2. **Backoff que cobre o cold start:** no 429, esperar **5s, 10s, 20s, 30s**
   (total ~65s), não 1s/2s. `status_forcelist=[429,502,503,504]`.
3. **Mensagem ao usuário** só após esgotar o backoff longo.

> **Fix definitivo (infra):** manter a BRCobrança **acordada** — UptimeRobot /
> cron-job.org pingando `/api/health` a cada **10 min**, ou upgrade para o plano
> Starter (sem sleep). Com o serviço quente, o 429 some.
